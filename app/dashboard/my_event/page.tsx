...
export default function MesEvenementsPage() {
    ...
    return (
        <div className="min-h-screen bg-black p-6">
            ...
            <div className="mb-8 opacity-0 animate-fadeIn" style={{ animationDelay: "200ms", animationFillMode: "forwards" }}>
                <h1 className="text-3xl font-bold text-white text-center mb-6">Événements</h1>
                ...
                {filteredEvents.map((event) => (
                    ...
                    >
                        ...
                        <div className="bg-neutral-900 p-5 flex-grow h-32 flex flex-col justify-center">
                            <h2 className="text-xl font-medium text-white mb-2 transition-all duration-300"
                                style={{ transform: hoveredId === event.id ? "translateX(6px)" : "translateX(0)" }}
                            >
                                {event.title}
                            </h2>
                            <p className="text-sm text-gray-400 mb-1 transition-all duration-300"
                               style={{ transform: hoveredId === event.id ? "translateX(3px)" : "translateX(0)" }}
                            >
                                {event.date}
                            </p>
                                ...
                                <Link href={`/evenement/${event.id}`}>
                                    <button
                                        className={`text-sm transition-all duration-300 relative ${
                                            hoveredId === event.id ? "text-blue-400 font-medium" : "text-white"
                                        }`}
                                    >
        ...
        @keyframes floatingParticle {
          ...
          50% { opacity: 1; }
          100% { transform: translateY(-40px) translateX(${Math.random() > 0.5 ? '+' : '-'}20px); opacity: 0; }
        }
